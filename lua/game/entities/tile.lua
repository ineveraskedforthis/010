...
function tile.Tile:new()
	...
	do
		...
		TILE.set_x(tile_dcon_id, pos_x / rr)
		TILE.set_y(tile_dcon_id, pos_y / rr)
		TILE.set_z(tile_dcon_id, pos_z / rr)
	end
...
function tile.province(id)
	...
	return TILE.province_membership_get_province(membership)
end
...
function tile.set_debug_color(tile_id, r, g, b)
	TILE.set_debug_r(tile_id, r);
	TILE.set_debug_g(tile_id, g);
	TILE.set_debug_b(tile_id, b);
end
...
function tile.set_real_color(tile_id, r, g, b)
	TILE.set_real_r(tile_id, r);
	TILE.set_real_g(tile_id, g);
	TILE.set_real_b(tile_id, b);
end
...
function tile.average_waterflow(tile_id)
	return (TILE.get_january_waterflow(tile_id) + TILE.get_july_waterflow(tile_id)) / 2
end
...
function tile.update_climate_data(tile_id)
	...
	-- print(tile_id, lat, lon)
	local r_ja, t_ja, r_ju, t_ju = require "game.climate.utils".get_climate_data(lat, lon, TILE.get_elevation(tile_id))

	TILE.set_january_rain(tile_id, r_ja)
	TILE.set_january_temperature(tile_id, t_ja)

	TILE.set_july_rain(tile_id, r_ju)
	TILE.set_july_temperature(tile_id, t_ju)
end
...
function tile.get_climate_data(tile_id)
	return TILE.get_january_rain(tile_id),
		TILE.get_january_temperature(tile_id),
		TILE.get_july_rain(tile_id),
		TILE.get_july_temperature(tile_id)
end
...
function tile.get_neighbor(tile_id, neighbor_index)
	return TILE.get_neighbour(tile_id, neighbor_index - 1);
end
...
function tile.soil_depth(tile_id)
	return TILE.get_sand(tile_id) + TILE.get_silt(tile_id) + TILE.get_clay(tile_id)
end
...
function tile.get_cartesian(tile_id)
	return TILE.get_x(tile_id), TILE.get_y(tile_id), TILE.get_z(tile_id)
end
...
function tile.is_coast(tile_id)
	if TILE.get_is_land(tile_id) then
		for n in tile.iter_neighbors(tile_id) do
			if not TILE.get_is_land(n) then
				return true
			end
		end
	else
		for n in tile.iter_neighbors(tile_id) do
			if TILE.get_is_land(n) then
				return true
			end
		end
	end
