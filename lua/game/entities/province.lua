...
function prov.Province.add_tile(province, tile)
	...
	if membership ~= INVALID_ID then
		TILE.province_membership_set_province(membership, province)
	else
		DATA.force_create_tile_province_membership(province, tile)
	end

	-- sanity check
	-- local belongs2 = TILE.province_membership_get_province(DATA.get_tile_province_membership_from_tile(tile)) == province
	...
	-- 	local check = TILE.province_membership_get_tile(item)
	-- 	assert(TILE.province_membership_get_province(DATA.get_tile_province_membership_from_tile(check)) == province)
	-- 	print("???")
	-- 	if tile == check then
	-- 		belongs = true
	-- 	end
...
function prov.Province.research(province, researched_technology)
	...
	for _, t in pairs(DATA.get_technology_unlock_from_origin(researched_technology)) do
		...
		for i = 1, MAX_REQUIREMENTS_TECHNOLOGY - 1 do
			...
			if TILE.get_biome(center) == required_biome then
	...
	for _, b in pairs(DATA.get_technology_building_from_technology(researched_technology)) do
		...
		for i = 1, MAX_REQUIREMENTS_BUILDING_TYPE - 1 do
			...
			if TILE.get_biome(DATA.province_get_center(province)) == required_biome then
				ok = true
				break
			end
		end
...
---@param pop pop_id
function prov.Province.add_pop(s, pop)
	POP_LOCATION.force_create(s, pop)
end
...
---@param s settlement_id
---@param pop pop_id
function prov.Province.set_home(s, pop)
	HOME.force_create(s, pop)
	local realm_rel = SETTLEMENT.get_realm_settlements(s)
	local realm = REALM_SETTLEMENTS.get_realm(realm_rel)
	REALM_POP.force_create(realm, pop)
end
...
function prov.Province.get_hiding(province)
	...
	DATA.for_each_tile_province_membership_from_province(province, function (item)
		...
		local grass = TILE.get_grass(tile_id)
		local shrub = TILE.get_shrub(tile_id)
		local conifer = TILE.get_conifer(tile_id)
		local broadleaf = TILE.get_broadleaf(tile_id)
