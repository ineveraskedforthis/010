...
local function calculate_province_neighbors(province, deep_logs)
	...
	for _, tile_membership_id in pairs(DATA.get_tile_province_membership_from_province(province)) do
		local tile_id = TILE.province_membership_get_tile(tile_membership_id)
		...
		for n in tile.iter_neighbors(tile_id) do
			...
			if tile.province(n) ~= province then
				TILE.set_is_border(tile_id, true)
...
function pro.run()
	...
	local function fill_out(strict_flag)
		while queue:length() > 0 do
			...
			if TILE.get_is_land(local_center) then
			...
			if size < expected_size * 1.1 then
				if TILE.get_is_land(tile_id) then
					...
					if love.math.random() < growth_probability then
						for n in tile.iter_neighbors(tile_id) do
							if tile.province(n) == INVALID_ID and TILE.get_is_land(n) == TILE.get_is_land(tile_id) then
								if (not strict_flag) or (math.abs(TILE.get_elevation(tile_id) - TILE.get_elevation(n)) < 350) then
					...
					if love.math.random() < growth_probability then
						for n in tile.iter_neighbors(tile_id) do
							if tile.province(n) == INVALID_ID and TILE.get_is_land(n) == TILE.get_is_land(tile_id) then
								if (not strict_flag) or (math.abs(TILE.get_elevation(tile_id) - TILE.get_elevation(n)) < 700) then
									pp.Province.add_tile(province, n)
									queue:enqueue(n)
								end
							end
	...
	local function coastal_recursion(tile_id, depth, province)
		...
		for n in tile.iter_neighbors(tile_id) do
			if TILE.get_is_land(n) == TILE.get_is_land(tile_id) then
	...
	local function river_search(tile_id, depth, searched)
		...
		for n in tile.iter_neighbors(tile_id) do
			if TILE.get_is_land(n) == TILE.get_is_land(tile_id) and not searched[n] then
	...
	for _ = 1, prov_count - used_provs do
		...
		local failsafe = 0
		while not TILE.get_is_land(tile_id) or tile.province(tile_id) ~= INVALID_ID or not check_neighs(tile_id) do
	...
	for _ = 1, prov_count do
		...
		local tile_id = WORLD:random_tile()
		while TILE.get_is_land(tile_id) or tile.province(tile_id) ~= INVALID_ID do
			tile_id = WORLD:random_tile()
		end
	...
	local function recalculate_provincial_centers()
		DATA.for_each_province(function (province)
			...
			for i = 1, N do
				...
				table.insert(sample, TILE.province_membership_get_tile(membership))
			end
	...
	DATA.for_each_province(function (province)
		...
		if TILE.get_is_land(DATA.province_get_center(province)) then
			...
			for _, tile_membership_id in pairs(DATA.get_tile_province_membership_from_province(province)) do
				...
				for n in tile.iter_neighbors(tile_id) do
					if tile.province(n) ~= INVALID_ID and tile.province(n) ~= province then
						...
						if TILE.get_is_land(neigh_center) == TILE.get_is_land(tile_id) then
							...
							if big_size + size < expected_size or size < 10 or distance < 0.005 then
								...
								for _, neigh_tile_membership_id in pairs(tiles_to_transfer) do
									local neigh_tile_id = TILE.province_membership_get_tile(neigh_tile_membership_id)
	...
	DATA.for_each_province(function (province)
		...
		for _, tile_membership_id in pairs(DATA.get_tile_province_membership_from_province(province)) do
			local tile_id = TILE.province_membership_get_tile(tile_membership_id)
			local biome = TILE.get_biome(tile_id)
