...
function dbm.total_production(tile_id)
	...
	if TILE.get_has_marsh(tile_id) then
		marine_production = marine_production + 0.5
	end
	if TILE.get_has_river(tile_id) then
		marine_production = marine_production + 0.5
	end
	for i = 1, 4 do
		if not TILE.get_is_land(tile.get_neighbor(tile_id, i)) then
...
function dbm.total_foraging_amounts(province)
	...
	accumulate = tabb.accumulate(
		tabb.map_array(
			...
			TILE.province_membership_get_tile
		),
		accumulate,
