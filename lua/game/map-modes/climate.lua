...
function cli.koppen()
	DATA.for_each_tile(function (tile_id)
		...
		local k = kopp.get_koppen(t_ja, t_ju, r_ja, r_ju, TILE.get_is_land(tile_id))
...
function cli.jan_flow()
	ut.simple_hue_map_mode(function(tile_id)
		---@type tile_id
		local tt = tile_id
		return math.pow(TILE.get_january_waterflow(tt), flow_map_mode_exponent) / math.pow(30000.0, flow_map_mode_exponent)
	end)
end
...
function cli.jul_flow()
	ut.simple_hue_map_mode(function(tile_id)
		---@type tile_id
		local tt = tile_id
		return math.pow(TILE.get_july_waterflow(tt), flow_map_mode_exponent) / math.pow(30000.0, flow_map_mode_exponent)
	end)
end
