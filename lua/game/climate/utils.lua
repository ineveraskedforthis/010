...
---@param index climate_cell_id
---@return number
---@return number
function ut.get_x_y(index)
	-- assert(index)
	-- assert(index == index)
	local x = index % WORLD.climate_grid_size;
	local y = math.floor(index / WORLD.climate_grid_size);
	return x, y
end
...
function ut.get_climate_data(lat, lon, elevation)
	...
	-- print(#WORLD.climate_cells);
	-- print(ac, acf, bc, bcf, cc, ccf, dc, dcf)
	...
	local function weighted_sum(getter)
		return getter(a) * acf
		+ getter(b) * bcf
		+ getter(c) * ccf
		+ getter(d) * dcf
	...
	local r_ja = weighted_sum(CLIMATE_CELL.get_january_rainfall)
	local t_ja = weighted_sum(CLIMATE_CELL.get_january_temperature)
	local r_ju = weighted_sum(CLIMATE_CELL.get_july_rainfall)
	local t_ju = weighted_sum(CLIMATE_CELL.get_july_temperature)
...
function ut.get_humidity_and_wind_speed(lat, lon)
	...
	return
		weighted_sum(CLIMATE_CELL.get_january_humidity),
		weighted_sum(CLIMATE_CELL.get_july_humidity),
		weighted_sum(CLIMATE_CELL.get_january_wind_speed),
		weighted_sum(CLIMATE_CELL.get_july_wind_speed)
end
