...
function sa.run()
	...
	for lua_x = 1, WORLD.climate_grid_size do
		...
		for lua_y = 1, WORLD.climate_grid_size do
			...
			local water_fraction = CLIMATE_CELL.get_water_fraction(cell)

			local val = (1 - water_fraction) * math.abs(math.sin(l)) / WORLD.climate_grid_size
			if l > 0 then
		...
		for lua_y = 1, WORLD.climate_grid_size do
			...
			CLIMATE_CELL.set_saldo_north(cell, local_saldo_north)
			CLIMATE_CELL.set_saldo_south(cell, local_saldo_south)
		end
	end
end
