	...
	for lua_y = 1, WORLD.climate_grid_size do
		...
		local function do_one(starting_x, ending_x, next_x_closure, set_cont, set_rs)
			...
			while not found_water and starting_x ~= ending_x do
				...
				if CLIMATE_CELL.get_water_fraction(i) > 0 then
					found_water = true
				else
			...
			if found_water then
				...
				local previous_ele = math.max(0, CLIMATE_CELL.get_elevation(starting_id))

				for lua_x = 1, WORLD.climate_grid_size do
					local x = lua_x - 1
					local true_x = next_x_closure(starting_x, x) % WORLD.climate_grid_size
					local cell = ut.get_id(true_x, y)
					local ele = math.max(0, CLIMATE_CELL.get_elevation(cell))
					...
					if CLIMATE_CELL.get_water_fraction(cell) > 0 then
						...
						current_rain_shadow = math.max(0,
							current_rain_shadow - CLIMATE_CELL.get_water_fraction(cell) * one_over_gs * RAIN_SHADOW_DECREASE_RATE)
		...
		-- LEFT TO RIGHT
		do_one(
			0, WORLD.climate_grid_size,
			function(x, d)
				return x + d
			end,
			CLIMATE_CELL.set_left_to_right_continentality,
			CLIMATE_CELL.set_left_to_right_rain_shadow
		)
		-- RIGHT TO LEFT
		do_one(
			WORLD.climate_grid_size - 1, -1,
			function(x, d)
				return x - d
			end,
			CLIMATE_CELL.set_right_to_left_continentality,
			CLIMATE_CELL.set_right_to_left_rain_shadow
		)
		...
		for lua_x = 1, WORLD.climate_grid_size do
			...
			local lr_shadow = CLIMATE_CELL.get_left_to_right_rain_shadow(cell)
			local rl_shadow = CLIMATE_CELL.get_right_to_left_rain_shadow(cell)
			CLIMATE_CELL.set_true_rain_shadow(cell, lr_shadow * left_to_right + rl_shadow * right_to_left)

			local lr_cont = CLIMATE_CELL.get_left_to_right_continentality(cell)
			local rl_cont = CLIMATE_CELL.get_right_to_left_continentality(cell)
			CLIMATE_CELL.set_true_continentality(cell, (lr_cont * left_to_right + rl_cont * right_to_left) * 2 / 3 ) -- don't ask why
		end
	end
end
